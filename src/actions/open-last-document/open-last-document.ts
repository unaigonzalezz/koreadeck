...
@action({ UUID: "com.unai-gonzalez.koreadeck.open-last-document" })
export class KoreaderOpenLastDocument extends SingletonAction {
    ...
    const response = await callKoreader(
      {
        ip,
        port,
        path: `/koreader/event/OpenLastDoc`,
      },
      timeoutMs,
    );
